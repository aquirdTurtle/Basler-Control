...
/*
void CameraImageDimensionsControl::drawBackgrounds( BaslerCameraWindow* camWin )
{
...
void CameraImageDimensionsControl::setImageParametersFromInput( imageParameters param, BaslerCameraWindow* camWin )
{
	...
	*/
	...
	for (int imageLocation = 0; imageLocation < eImageBackgroundAreas.size(); imageLocation++)
	{
		...
		double boxWidth = imageBoxWidth / (double)currentImageParameters.horBinNumber;
		double boxHeight = imageBoxHeight / (double)currentImageParameters.vertBinNumber;
		if (boxWidth > boxHeight)
		{
			// scale the box horBinNumber down.
		...
		{
			// cale the box vertBinNumber down.
			eImageDrawAreas[imageLocation].left = eImageBackgroundAreas[imageLocation].left;
	...
	{
		ePixelRectangles[pictureInc].resize(currentImageParameters.horBinNumber);
		for (int widthInc = 0; widthInc < currentImageParameters.horBinNumber; widthInc++)
		{
			ePixelRectangles[pictureInc][widthInc].resize(currentImageParameters.vertBinNumber);
			for (int heightInc = 0; heightInc < currentImageParameters.vertBinNumber; heightInc++)
			{
				// for all 4 pictures...
				ePixelRectangles[pictureInc][widthInc][heightInc].left = (int)(eImageDrawAreas[pictureInc].left
					...
					/ (double)currentImageParameters.horBinNumber + 2);
				...
				ePixelRectangles[pictureInc][widthInc][heightInc].top = (int)(eImageDrawAreas[pictureInc].top
					...
					/ (double)currentImageParameters.vertBinNumber);
...
HBRUSH CameraImageDimensionsControl::colorEdits( HWND window, UINT message, WPARAM wParam, LPARAM lParam, BaslerCameraWindow* mainWin )
{
	...
	else if (controlID == verticalBinningEdit.ID)
	{
		...
		try
		{
			...
			if (verticalBin == currentImageParameters.vertPixelsPerBin)
			{
				// good.
				SetTextColor( hdcStatic, RGB( 255, 255, 255 ) );
				SetBkColor( hdcStatic, RGB( 100, 110, 100 ) );
	...
	else if (controlID == horizontalBinningEdit.ID)
	{
		...
		try
		{
			...
			if (horizontalBin == currentImageParameters.horPixelsPerBin)
			{
				// good.
				SetTextColor( hdcStatic, RGB( 255, 255, 255 ) );
				SetBkColor( hdcStatic, RGB( 100, 110, 100 ) );
