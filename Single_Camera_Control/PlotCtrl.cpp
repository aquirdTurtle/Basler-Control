	...
	brushes( plotBrushes ), pens( pensIn ), narrow( narrowOpt )
{
	...
	Gdiplus::Color blue( 255, 50, 50, 255 );
	singleBrush = new Gdiplus::SolidBrush( blue );
	singlePen = new Gdiplus::Pen( blue );
...
void PlotCtrl::convertDataToScreenCoords( std::vector<plotDataVec>& screenData )
{
	...
	if ( style == plotStyle::HistPlot || style == plotStyle::VertHist )
	{
...
void PlotCtrl::plotPoints( memDC* d )
{
	...
	for ( auto& line : shiftedData )
	{
		...
		{
			// only one line, special.
			pen = singlePen;
			brush = singleBrush;
		}
		else if ( lineNum == shiftedData.size( ) - 1 && style == plotStyle::ErrorPlot )
		{
	...
	for ( auto& point : scaledLine )
	{
		...
		if ( style == plotStyle::HistPlot )
		{
			r = Gdiplus::Rect(floor( point.x - boxWidthPixels / 2 ), point.y, boxWidthPixels,
				  floor( plotAreaDims.bottom * heightScale2 - point.y ));
		}
